...
public class CMWinClientEventHandler implements CMEventHandler{
	...
	private static CMWinClient m_client;
	...
	private void processSessionEvent(CMEvent cme)
	{
		...
		case CMSessionEvent.SESSION_TALK:
			...
			//printMessage("<"+se.getUserName()+">:  "+se.getHandlerGroup()+"\n");
			
			SendEventToEsper(se.getUserName(),se.getTalk(),se.getCurrentGroupName());
	...
	//Send Chatting message to Esper engine
	private void SendEventToEsper(String userName, String talk,String GroupName) {
		String[] chatMessage = talk.split(" "); 
		for(int i=0;i<chatMessage.length;i++){
			ChatEventType ChatEvent = new ChatEventType();
			ChatEvent.setUserName(userName);
			ChatEvent.setChatMessage(chatMessage[i]);
			ChatEvent.setGroupName(GroupName);
			
			runtime.sendEvent(ChatEvent);
			System.out.println(ChatEvent);
		}
	}
	public static void PushTheGroupInfo(String Message) {
		// check local state
		CMInteractionInfo interInfo = m_clientStub.getCMInfo().getInteractionInfo();
		...
		while (iter.hasNext()) {
			...
			if (Message.equals(gInfo.getGroupName())) {
				printMessage("You may want to join this group \n");
				printMessage("---------------------------------------------------------\n");
	...
	private void processInterestEvent(CMEvent cme)
	{
		...
		case CMInterestEvent.USER_TALK:
			...
			SendEventToEsper(ie.getUserName(),ie.getTalk(),ie.getHandlerGroup());
			break;
		default:
			return;
		}
	...
	private static void printMessage(String strText)
	{
		/*
